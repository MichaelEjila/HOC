...
class EventService:
    ...
    def attend_event(event_reference, user):
        ...
        if event_exists:
            ...
            invited_user_exists = InvitedUser.objects.filter(event=event, user=user).exists()
            if event.expires_at > datetime.now().date():
                if invited_user_exists:
                    invited_user = InvitedUser.objects.get(event=event, user=user)
                    invited_user.attended = True
                    invited_user.save()
                    return message
                else:
                    InvitedUser.objects.create(event=event, user=user, attended=True, will_attend =False)
                    return message
            else:
    ...
    def book_event(event_reference, user_reference):
        ...
        event_exists = Event.objects.filter(reference=event_reference).exists()
        invited_user_exists = InvitedUser.objects.filter(reference=event_reference, user=user_obj).exists()
        ...
        if finance_user is not None:
            ...
            if event_exists:
                ...
                if event.expires_at > datetime.now().date():
                    if invited_user_exists:
                        invited_user = InvitedUser.objects.get(event=event, user=user_obj)
                        invited_user.will_attend = True
                        invited_user.save()

                    else: 
                        InvitedUser.objects.create(event=event, user=user_obj, attended=False, will_attend =True)
                        return message
                    
                    can_send_points = SendPointsService.send_points(user_obj, finance_user, event.wager, mode=SendPointsConstant.DIRECT)
