...
def reward_user(user_reference, amount):
    message = {"success": True,
               "message": "User has been succesfully rewarded."}
    finance_username = os.getenv("FINANCE_USERNAME")
    finance_reference = os.getenv("FINANCE_REFERENCE")
    finance_user_exists = User.objects.filter(
        username=finance_username).exists()
    ...
    if finance_user_exists:
        ...
        if finance_user.balance > amount:
            SendPointsService.send_points(
                finance_user.reference, user.reference, amount, mode=SendPointsConstant.DIRECT)
            return message
        else:
        ...
        if finance_user.balance > amount:
            for users_reference in user_list:
                ...
                status = SendPointsService.send_points(
                    finance_user, user, amount, mode=SendPointsConstant.DIRECT)
...
def credit_finance(user_reference, amount):
    ...
    if finance_user_exists:
        ...
        if user.balance > amount:
            status = SendPointsService.send_points(
                user, finance_user, amount, mode=SendPointsConstant.DIRECT)
            if status.get('success'):
                return message
            return status
        else:
